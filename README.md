...
No additional entries needed. Verify the generated manifest in the `.ipa` via `unzip -p Repause.ipa Payload/Repause.app/PrivacyInfo.xcprivacy`.

## Submission checklist

Things the developer must still do manually:

- [ ] Create Apple Developer account entry and App ID `com.tomasmach.repause` in App Store Connect.
...
- [ ] Fill App Store Connect metadata: name "Repause", subtitle, description, keywords, primary category Health & Fitness, secondary Utilities, age rating 4+.
...
2. Build a native iOS dev client: `npm run ios:dev` (runs `npx expo run:ios`). This compiles a `.app` with bundle id `com.tomasmach.repause` and installs it in the simulator.
